...
func (s *Store) Get(ctx context.Context, token string) (shared.Phone, error) {
	...
	if errors.Is(err, redis.Nil) {
		return shared.Phone{}, authDomain.ErrResetTokenNotFound
	}
	if err != nil {
		return shared.Phone{}, fmt.Errorf("get reset token: %w", err)
	}

	phone, err := shared.NewPhone(string(data))
	if err != nil {
		return shared.Phone{}, fmt.Errorf("parse phone from reset token: %w", err)
	}
